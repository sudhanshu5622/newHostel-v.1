...
function ResetPassword() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setStep(2);
        setMessage("OTP sent! Check your email to proceed.");
      } else setMessage(response.data.message || "Failed to send OTP.");
    } catch (error) {
      setMessage(error.response?.data?.message || 'Error: Could not send OTP request.');
    }
    setLoading(false);
  ...
  const handleResetPassword = async () => {
    ...
    if (!otp || !newPassword) return setMessage("Please enter the OTP and a new password.");
    setLoading(true); setMessage('');
    try {
      ...
      if (response.data.success) {
        setIsSuccess(true);
        setStep(0);
      } else setMessage(response.data.message || 'Password reset failed.');
    } catch (error) {
      setMessage('Reset failed: ' + (error.response?.data?.message || error.message));
    }
    setLoading(false);
  ...
  return (
    <div className="reset-password-wrapper">
      <div className="reset-password-card">
        ...
        <p>{step === 1 ? "Enter your email to receive a password reset code." : "Enter the received code and your new password."}</p>
          ...
          <div>
            {step === 1 && (
              <>
                ...
                {loading && <div className="loader-container"><LoaderDots /></div>}
                <button onClick={handleSendOtp} disabled={loading || !email} className="send-otp-btn">Send Reset Code</button>
              </>
            )}

            {step === 2 && (
              <>
                <input type="text" placeholder="6-Digit Reset Code" value={otp} onChange={e => setOtp(e.target.value)} />
                <input type="password" placeholder="New Password" value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                <input type="password" placeholder="Confirm New Password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
                <button onClick={handleResetPassword} disabled={loading || !otp || !newPassword || newPassword !== confirmPassword} className="reset-btn">Reset Password</button>
              </>
            )}

            {message && <p className={`reset-password-message ${isSuccess ? 'success' : 'error'}`}>{message}</p>}
          </div>
        )}
      </div>
    </div>
